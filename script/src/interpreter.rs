...
mod tests {
	...
	fn test_script_with_forkid_signature() {
		...
		let key_pair = KeyPair::from_private(Private { network: Network::BtcMainnet, secret: 1.into(), compressed: false, }).unwrap();
	...
	fn checkdatasig_spec_tests() {
		...
		let kp = KeyPair::from_private(Private { network: Network::BtcMainnet, secret: 1.into(), compressed: false, }).unwrap();
